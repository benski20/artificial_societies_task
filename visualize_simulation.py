"""
Visualize simulation results - belief changes and survey response evolution.
"""
...
def load_simulation_results(results_dir: Path):
    ...
    survey_file = results_dir / "multi_round_survey_evolution.json"
    
    with open(survey_file, 'r') as f:
        survey_evolution = json.load(f)
    
    return survey_evolution
...
def plot_survey_evolution(survey_evolution: dict, output_dir: Path):
    ...
    for question_id, evolution in survey_evolution.items():
        ...
        ax.set_title(f'Survey Response Evolution: {question_text}', fontsize=14, fontweight='bold')
...
def plot_combined_evolution(survey_evolution: dict, output_dir: Path):
    """Plot all survey questions in one figure."""
    
    n_questions = len(survey_evolution)
    fig, axes = plt.subplots(n_questions, 1, figsize=(14, 5 * n_questions))
    
    if n_questions == 1:
        axes = [axes]
    
    for idx, (question_id, evolution) in enumerate(survey_evolution.items()):
        question_text = SURVEY_QUESTIONS[question_id]['question']
        options = SURVEY_QUESTIONS[question_id]['options']
        ...
        ax.set_title(question_text, fontsize=12, fontweight='bold')
        ax.legend(loc='best', fontsize=9)
        ax.grid(True, alpha=0.3)
        ax.set_xticks(rounds)
    
    plt.tight_layout()
    fig.savefig(output_dir / "all_surveys_evolution.png", dpi=150, bbox_inches='tight')
    plt.close(fig)
    print(f"✓ Saved all_surveys_evolution.png")
...
def plot_change_summary(survey_evolution: dict, output_dir: Path):
    ...
    ax.set_title('Survey Response Changes Over Simulation', fontsize=14, fontweight='bold')
...
def main():
    ...
    results_dir = Path('outputs/simulation')
    
    if not (results_dir / "multi_round_survey_evolution.json").exists():
    ...
    survey_evolution = load_simulation_results(results_dir)
    
    # Create output directory with timestamp
    from datetime import datetime
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    viz_dir = Path('outputs') / f"simulation_viz_{timestamp}"
    ...
    # Combined plot
    plot_combined_evolution(survey_evolution, viz_dir)
